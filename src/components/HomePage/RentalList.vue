		...
		<div id="tabel">
			<Table :columns="tablehead" :data="equipdata">
				<template slot-scope="{ row }" slot="status">
					<Button v-if="row.status === 'onsale'" type="success" size="small" style="margin-right: 5px" >空闲</Button>
					<Button v-else-if="row.status === 'rented'" type="error" size="small" style="margin-right: 5px" >占用</Button>
...
export default {
	data() {
		...
		], equipdata: [], visiable: false, apply: {}, page: 1, page_size: 999999, total: 0 }
	},
	mounted: function() {
		this.loadEquipments()
	},
